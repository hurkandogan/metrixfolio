...
export default function PositionsPage() {
  ...
  const portfolioAssets = assets.filter((a) => a.category_id !== 'uncategorized');
  ...
  const totalMarketValue = portfolioAssets.reduce((sum, asset) => sum + (asset.market_value || 0), 0);
  const totalUnrealizedPnl = portfolioAssets.reduce((sum, asset) => sum + (asset.unrealized_pnl || 0), 0);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-6">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="form-control">
                  ...
                  <input required type="number" step="any" className="input input-bordered" value={editForm.amount} onChange={(e) => setEditForm({ ...editForm, amount: e.target.value })} />
                </div>
                <div className="form-control">
                  <label className="label"><span className="label-text">Avg Cost (Unit)</span></label>
              ...
              {!selectedAsset.close_date && (
                <div className="form-control">
                  ...
                  <select className="select select-bordered w-full" value={editForm.category_id} onChange={(e) => setEditForm({ ...editForm, category_id: e.target.value })}>
                    ...
                    {categories.map((cat) => (
                      <option key={cat.id} value={cat.id}>{cat.name} ({cat.target_percentage}%)</option>
                    ))}
                  </select>
                </div>
              )}
      ...
      <dialog ref={closePortalRef} className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          <h3 className="font-bold text-lg flex items-center gap-2 text-primary">
            <FiCheckCircle /> Close Position
          </h3>
          <p className="py-4 text-sm opacity-70">Enter the selling price per unit for <b>{selectedAsset?.symbol}</b> to realize your PnL.</p>
          <form onSubmit={handleClosePosition} className="space-y-4">
            <div className="form-control">
              <label className="label"><span className="label-text font-bold">Close Price (Unit)</span></label>
              <input required type="number" step="any" className="input input-bordered focus:input-primary" value={closeFormData.price} onChange={(e) => setCloseFormData({ price: e.target.value })} autoFocus />
            </div>
            <div className="modal-action">
              <button type="button" className="btn btn-ghost" onClick={() => closePortalRef.current?.close()}>Cancel</button>
              <button type="submit" className="btn btn-primary px-8">Confirm & Close</button>
            </div>
          </form>
        </div>
        <form method="dialog" className="modal-backdrop"><button>close</button></form>
      ...
      {currentTab === 'OPEN' && inboxAssets.length > 0 && (
        <div className="card bg-warning/10 border-warning/20 border shadow-sm">
          <div className="card-body">
            ...
            <p className="mb-4 text-sm opacity-80">Assign these to a category to include them in your strategy.</p>
              ...
              <table className="table-zebra table">
                ...
                <tbody>
                  {inboxAssets.map((asset) => (
                    <tr key={asset.id}>
                      <td><div className="badge badge-ghost text-xs font-bold">{asset.source}</div></td>
                      <td>
                      ...
                      <td className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button className="btn btn-xs btn-outline btn-primary" onClick={() => openCloseModal(asset)}>Close</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <div className="card bg-base-100 border-base-300 border shadow-xl">
        <div className="card-body p-0">
          <div className="border-base-200 border-b p-6 flex items-center justify-between">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search..." className="input input-bordered input-sm w-full max-w-xs pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
              ...
              <table className="table">
                <thead>
                  <tr className="bg-base-200/50">
                    ...
                    <th className="cursor-pointer hover:bg-base-200 text-right" onClick={() => handleSort('market_value')}>Value {renderSortIcon('market_value')}</th>
                ...
                <tbody>
                  {filteredAndSortedAssets.map((asset) => (
                    <tr key={asset.id} className="hover">
                      <td><span className={`badge text-xs font-bold ${asset.source === 'IBKR' ? 'badge-error text-white' : 'badge-info text-white'}`}>{asset.source}</span></td>
                  ...
                  {filteredAndSortedAssets.length === 0 && (
                    <tr><td colSpan={9} className="py-8 text-center opacity-50">No active assets found.</td></tr>
                  )}
                </tbody>
                {portfolioAssets.length > 0 && (
                  <tfoot>
                    <tr className="bg-base-200/50 font-bold text-base-content">
                      ...
                      <td className="text-right font-mono text-lg">{formatMoney(totalMarketValue.toString(), 'USD')}</td>
                      <td className={`text-right font-mono text-lg ${totalUnrealizedPnl >= 0 ? 'text-success' : 'text-error'}`}>{formatMoney(totalUnrealizedPnl.toString(), 'USD')}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                )}
              ...
              <table className="table">
                ...
                <tbody>
                  {filteredClosedAssets.map((asset) => (
                    <tr key={asset.id} className="hover">
                      <td className="text-xs opacity-70">{new Date(asset.close_date * 1000).toLocaleDateString()}</td>
                      <td><span className={`badge badge-xs text-[10px] font-bold ${asset.source === 'IBKR' ? 'badge-error text-white' : 'badge-info text-white'}`}>{asset.source}</span></td>
                      <td><div className="font-bold">{asset.symbol}</div><div className="text-[10px] opacity-50">{asset.name}</div></td>
                      <td className="text-right font-mono">{asset.amount}</td>
                      <td className="text-right font-mono small">{formatMoney(asset.avg_cost.toString(), 'USD')}</td>
                      <td className="text-right font-mono small text-primary">{formatMoney(asset.close_price.toString(), 'USD')}</td>
